...
That loop comes back twice in what follows. It is what a brain-inspired model runs when it recognizes an object by moving over it, and it is where the precision accounts of psychiatric conditions attach. The search only works because an empty spot counts as evidence and shifts the guess, and that is the part those accounts change.
